...
it('agents: default field accessors read Claude names AND alternatives', () => {
  ...
  assert.strictEqual(f.sessionId({ sessionId: 'B' }), 'B');
  assert.strictEqual(f.sessionId({ conversation_id: 'C' }), 'C', 'cursor-style id');
  assert.strictEqual(f.sessionId({}), 'unknown', 'no id → unknown');
  assert.strictEqual(f.toolName({ tool_name: 'Write' }), 'Write');
  assert.strictEqual(f.toolName({ toolName: 'Edit' }), 'Edit');
  assert.strictEqual(f.toolName({ tool: 'edit_file' }), 'edit_file');
  assert.strictEqual(f.toolName({}), '');
  assert.deepStrictEqual(f.toolInput({ tool_input: { a: 1 } }), { a: 1 });
  assert.deepStrictEqual(f.toolInput({ toolInput: { b: 2 } }), { b: 2 });
  assert.deepStrictEqual(f.toolInput({ input: { c: 3 } }), { c: 3 });
  assert.deepStrictEqual(f.toolInput({ arguments: { d: 4 } }), { d: 4 });
  assert.deepStrictEqual(f.toolInput({}), {});
  assert.strictEqual(f.filePath({ file_path: '/a' }), '/a');
  assert.strictEqual(f.filePath({ path: '/b' }), '/b');
  assert.strictEqual(f.filePath({ filePath: '/c' }), '/c');
  assert.strictEqual(f.filePath({ target_file: '/x.js' }), '/x.js', 'cursor write target');
  assert.strictEqual(f.filePath({}), null);
  assert.strictEqual(f.prompt({ prompt: 'hi' }), 'hi');
  assert.strictEqual(f.prompt({ user_prompt: 'hey' }), 'hey');
  assert.strictEqual(f.prompt({ message: 'yo' }), 'yo');
  assert.strictEqual(f.prompt({ text: 'sup' }), 'sup');
  assert.strictEqual(f.prompt({}), '');
  assert.strictEqual(f.subAgentId({ agent_id: 'a1' }), 'a1');
  assert.strictEqual(f.subAgentId({ agentId: 'a2' }), 'a2');
  assert.strictEqual(f.subAgentId({ sub_agent_id: 'a3' }), 'a3');
  assert.strictEqual(f.subAgentId({}), null);
  assert.strictEqual(f.subAgentType({ agent_type: 't1' }), 't1');
  assert.strictEqual(f.subAgentType({ agentType: 't2' }), 't2');
  assert.strictEqual(f.subAgentType({ sub_agent_type: 't3' }), 't3');
  assert.strictEqual(f.subAgentType({}), null);
  assert.strictEqual(f.transcriptPath({ transcript_path: '/a' }), '/a');
  assert.strictEqual(f.transcriptPath({ transcriptPath: '/b' }), '/b');
  assert.strictEqual(f.transcriptPath({ transcript: '/c' }), '/c');
  assert.strictEqual(f.transcriptPath({}), null);
  assert.strictEqual(f.command({ command: 'ls' }), 'ls');
  assert.strictEqual(f.command({ cmd: 'pwd' }), 'pwd');
  assert.strictEqual(f.command({ script: 'echo' }), 'echo');
  assert.strictEqual(f.command({}), '');
  assert.strictEqual(f.pattern({ pattern: '*.js' }), '*.js');
  assert.strictEqual(f.pattern({ query: 'foo' }), 'foo');
  assert.strictEqual(f.pattern({ regex: 'bar' }), 'bar');
  assert.strictEqual(f.pattern({ glob: '**' }), '**');
  assert.strictEqual(f.pattern({}), null);
  assert.strictEqual(f.url({ url: 'https://a' }), 'https://a');
  assert.strictEqual(f.url({ uri: 'https://b' }), 'https://b');
  assert.strictEqual(f.url({ query: 'search' }), 'search');
  assert.strictEqual(f.url({}), null);
  assert.strictEqual(f.exitCode({ exit_code: 0 }), 0);
  assert.strictEqual(f.exitCode({ exitCode: 1 }), 1);
  assert.strictEqual(f.exitCode({ code: 2 }), 2);
...
it('agents: antigravity field accessors fall back to flat fields when toolCall is absent', () => {
  const a = agents.getAdapter('antigravity');
  assert.strictEqual(a.fields.toolName({ tool_name: 'view_file' }), 'view_file');
  assert.strictEqual(a.fields.toolName({ toolName: 'run_command' }), 'run_command');
  assert.strictEqual(a.fields.toolName({ tool: 'grep_search' }), 'grep_search');
  assert.strictEqual(a.fields.toolName({}), '');
  assert.deepStrictEqual(a.fields.toolInput({ tool_input: { x: 1 } }), { x: 1 });
  assert.deepStrictEqual(a.fields.toolInput({ toolInput: { y: 2 } }), { y: 2 });
  assert.deepStrictEqual(a.fields.toolInput({ input: { z: 3 } }), { z: 3 });
  assert.deepStrictEqual(a.fields.toolInput({ arguments: { w: 4 } }), { w: 4 });
  assert.deepStrictEqual(a.fields.toolInput({}), {});
  assert.deepStrictEqual(a.fields.toolResponse({ tool_response: { ok: true } }), { ok: true });
  assert.deepStrictEqual(a.fields.toolResponse({ tool_result: { ok: true } }), { ok: true });
  assert.deepStrictEqual(a.fields.toolResponse({ toolResponse: { ok: true } }), { ok: true });
  assert.deepStrictEqual(a.fields.toolResponse({ result: { ok: true } }), { ok: true });
  assert.deepStrictEqual(a.fields.toolResponse({}), {});
  assert.strictEqual(a.fields.transcriptPath({ transcript_path: '/a' }), '/a');
  assert.strictEqual(a.fields.transcriptPath({ transcript: '/c' }), '/c');
  assert.strictEqual(a.fields.transcriptPath({}), null);
});
